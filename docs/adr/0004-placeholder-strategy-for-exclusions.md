...
- 현재 범위(front matter, fenced code, block math, inline code, URL)는 정규식 기반으로도 충분히 식별 가능하며, 구현을 순수 함수로 유지하기 쉽다.
...
- Front matter, 펜스 코드 블록, block math(`$$ ... $$`), 인라인 코드, Markdown 링크의 URL 부분이 Exclusion에 해당한다. 새로운 Exclusion 타입 추가는 `MarkdownPreprocessor`만 수정하면 된다.
